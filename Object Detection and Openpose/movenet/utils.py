...
def visualize_learn(DISPLAY,
  ...
  return image, start_x, start_y, end_x, end_y

# x_list = []
# y_list = []

def visualize_observe(DISPLAY, cosine,
    image: np.ndarray,
    list_persons: List[Person],
    keypoint_color: Tuple[int, ...] = None,
    keypoint_threshold: float = 0.05,
  ...
  if person.score > instance_threshold:
    ...
    midpoint_x = int((start_point.x + end_point.x)/2)

    midpoint_y = int((start_point.y + end_point.y)/2)

    # x_list.append(midpoint_x)
    # y_list.append(midpoint_y)

    #print(x_list)
    #print(y_list)

    # if len(x_list) > 1:
    #   previous_x_coordinate = x_list[-2]
    #   previous_y_coordinate = y_list[-2]
    # else:
    #   previous_x_coordinate = 1
    #   previous_y_coordinate = 1

    # x_change = ((midpoint_x - previous_x_coordinate) / previous_x_coordinate)
    # y_change = ((midpoint_y - previous_y_coordinate) / previous_y_coordinate)
    ...
    if cosine >= 0.5 and person.id == 1:
      midpoint_follow_x = midpoint_x
      midpoint_follow_y = midpoint_y

      if DISPLAY == 1:
        cv2.circle(image, (midpoint_follow_x,midpoint_follow_y ), 2, (0, 255, 255), 4)
        cv2.putText(image, "follow me", (midpoint_follow_x,midpoint_follow_y), cv2.FONT_HERSHEY_PLAIN, 3,
                      (0, 255, 255), 1)
    else:
      midpoint_follow_x = 0
      midpoint_follow_y = 0
  ...
  return image, start_x, start_y, end_x, end_y, midpoint_follow_x, midpoint_follow_y
